...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
            ...
            if (pid.pgcateg() == BTreePageId.LEAF) {
                ...
                BTreeEntry current = null;
                Iterator<BTreeEntry> it = page.iterator();
                while(it.hasNext()) {
                    current = it.next();
                    if (f == null || f.compare(Op.LESS_THAN_OR_EQ, current.getKey())) {
                        return findLeafPage(tid, dirtypages, current.getLeftChild(), Permissions.READ_ONLY, f);
                    }
                }
                return findLeafPage(tid, dirtypages, current.getRightChild(), Permissions.READ_ONLY, f);
            }
	}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
            ...
            for (int count = 0; count < half; count++){
                ...
                rightneighbor.insertTuple(current);
            }
            rightneighbor.setRightSiblingId(page.getRightSiblingId());
            if (page.getRightSiblingId() != null) {
                BTreeLeafPage oldRightNeighbor = (BTreeLeafPage) getPage(tid, dirtypages, page.getRightSiblingId(), Permissions.READ_WRITE);
            ...
            Field key = current.getField(keyField);
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
			BTreeInternalPage parent, BTreeEntry entry, boolean isRightSibling) throws DbException {
                ...
                entry.setKey(current.getField(keyField));
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		// some code goes here
                ...
                BTreeEntry current = new BTreeEntry(parentEntry.getKey(), null, page.iterator().next().getLeftChild());
                for (int j = 0; j < toMove; j ++) {
                    BTreeEntry l = it.next();
                    current.setLeftChild(l.getRightChild());
                    page.insertEntry(current);
                    current = new BTreeEntry(l.getKey(), null, l.getRightChild());
                    leftSibling.deleteKeyAndRightChild(l);
                }
                parentEntry.setKey(current.getKey());
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
			...
			BTreeEntry parentEntry) throws DbException, IOException, TransactionAbortedException {
                ...
                BTreeEntry current = new BTreeEntry(parentEntry.getKey(), page.reverseIterator().next().getRightChild(), null);
                
                for (int i = 0; i < toMove; i ++) {
                    BTreeEntry r = it.next();
                    current.setRightChild(r.getLeftChild());
                    page.insertEntry(current);
                    current = new BTreeEntry(r.getKey(), r.getLeftChild(), null);
                    rightSibling.deleteKeyAndLeftChild(r);
                }
                parentEntry.setKey(current.getKey());
                parent.updateEntry(parentEntry);
                updateParentPointers(tid, dirtypages, page);
